...
INITIAL_PRESSURE: float = ATM  # [Pa]
INITIAL_SATURATION: float = 0.9  # [-], initial saturation. Domain filled with water.

INJECTION_RATE: float = 1.75e-7 / _co2["density"]  # 1.7x10^-5 kg/s
WELL_1_POS: tuple[float, float] = (0.9, 0.3)  # [m]
WELL_2_POS: tuple[float, float] = (1.7, 0.7)  # [m]

water: dict[str, Any] = _water.copy()
water.update(
    {
        # In the SPE11 model, the residual saturation varies between 0.1 and 0.32,
...
class EquationsSPE11(TPFProtocol):
    ...
    def phase_fluid_source(self, g: pp.Grid, phase: FluidPhase) -> np.ndarray:  # type: ignore
        ...
        if phase.name == self.nonwetting.name:
            ...
            # Upper well.
            array[
                cell_id_position(g, WELL_1_POS[0], WELL_1_POS[1], percentages=False)
            ] = phase.convert_units(INJECTION_RATE, "m^3") / phase.convert_units(
                pp.SECOND, "s"
            )
            # Lower well.
            array[
                cell_id_position(g, WELL_2_POS[0], WELL_2_POS[1], percentages=False)
            ] = phase.convert_units(INJECTION_RATE, "m^3") / phase.convert_units(
                pp.SECOND, "s"
            )
            return array
...
class ModifiedBoundarySPE11(TPFProtocol):

    def bc_type(self, g: pp.Grid) -> pp.BoundaryCondition:
        """BC type (Dirichlet or Neumann).

        Dirichlet bc at the top. No flow Neumann bc at the bottom, left and right.
...
class ModelGeometrySPE11(TPFProtocol):
...
# the error.
class SPE11Mixin(EquationsSPE11, ModifiedBoundarySPE11, SolutionStrategySPE11, ModelGeometrySPE11): ...  # type: ignore
