...
- No flow boundary condition on the sides and bottom.  Dirichlet on top.
...
class AccuracyMixin(TPFProtocol):
...
class SPE11Newton(AccuracyMixin, SPE11Mixin, TwoPhaseFlowANewton): ...


class SPE11AHC(AccuracyMixin, SPE11Mixin, TwoPhaseFlowAHC): ...
...
params: dict[str, Any] = {
    ...
    "rel_perm_constants": {
        "model": "Brooks-Corey",
        # Limit rel. perms. to avoid negative saturations.
        "limit": False,
        "n1": 2,
        "n2": 2,  # 1 + 2/n_b
        "n3": 1,
...
for time_step in np.logspace(1, -3, 5) * pp.DAY:
    ...
    model = SPE11Newton(params)  # type: ignore
    try:
...
for i, (
    ...
    model = (
        SPE11Newton(params)  # type: ignore
        if solver == "Newton"
        else SPE11AHC(params)  # type: ignore
    )
    pp.run_time_dependent_model(model=model, params=params)
