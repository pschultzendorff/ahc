...
class SPE10Newton(AccuracyMixin, SPE10Mixin, TwoPhaseFlowANewton): ...


class SPE10AHC(AccuracyMixin, SPE10Mixin, TwoPhaseFlowAHC): ...
...
for time_step in np.logspace(1, -3, 5) * pp.DAY:
    ...
    model = SPE10Newton(params)  # type: ignore
    try:
...
for i, (
    ...
    model = (
        SPE10Newton(params)  # type: ignore
        if solver == "Newton"
        else SPE10AHC(params)  # type: ignore
    )
    pp.run_time_dependent_model(model=model, params=params)
