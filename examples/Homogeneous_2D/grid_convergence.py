r"""Homogenous five-spot example.

Model description:
- 200x200 m domain.
- Constant water injection in the center: 87.5 m^3/day
- Oil production at the four corners: 3e7 Pa bhp
...
- Initial values:
    - Pressure: 4e7 Pa.
    - Saturation: 0.25, slightly above residual water saturation (0.2).
- Rel. perm. models:
    - linear
    - Corey with power 2.
...
from tpf.models.phase import FluidPhase, PhaseConstants
...
# region MODEL
BHP: float = 3e7  # Bottom hole pressure in Pa.


class ModifiedGeometry(pp.ModelGeometry):

    def set_domain(self) -> None:
        r"""2D domain of :math:`\qty{200 x 200}{\meter}`."""
...
class ModifiedEquations(EquationsTPF):
    ...
    def phase_fluid_source(self, g: pp.Grid, phase: FluidPhase) -> np.ndarray:
        ...
        if phase.name == self.wetting.name:
            ...
            if np.isclose(self.params["meshing_arguments"]["cell_size"], 200 / 7.5):
                array[128] = 87.5 / pp.DAY  # 87.5 m^3/day in [m^3/s]
            elif np.isclose(self.params["meshing_arguments"]["cell_size"], 200 / 15):
                array[500] = 87.5 / pp.DAY  # 87.5 m^3/day in [m^3/s]
            elif np.isclose(self.params["meshing_arguments"]["cell_size"], 200 / 30):
                array[1358] = 87.5 / pp.DAY  # 87.5 m^3/day in [m^3/s]
            elif np.isclose(self.params["meshing_arguments"]["cell_size"], 200 / 60):
                array[5173] = 87.5 / pp.DAY  # 87.5 m^3/day in [m^3/s]
            elif np.isclose(self.params["meshing_arguments"]["cell_size"], 200 / 120):
                array[3000] = 87.5 / pp.DAY  # 87.5 m^3/day in [m^3/s]
            return array
...
class ModifiedSolutionStrategyMixin:
    ...
    def initial_condition(self) -> None:
        ...
        initial_pressure = np.full(g.num_cells, 4e7)
        initial_pressure[corner_cell_ids] = BHP
        initial_saturation = np.full(g.num_cells, 0.25)
...
params = {
    ...
    "material_constants": {
        WETTING: PhaseConstants(water),
        NONWETTING: PhaseConstants(oil),
        "solid": pp.SolidConstants({"porosity": 0.1, "permeability": 1e-15}),
    },
    "rel_perm_constants": {},
    "cap_press_constants": {},
}
