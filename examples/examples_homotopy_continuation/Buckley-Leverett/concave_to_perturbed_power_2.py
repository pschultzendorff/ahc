...
In comparison to e.g., ``linear_to_perturbated_power_1.py``, the perturbations are less
in this script.
...
base_foldername: str = os.path.join(
    ...
    f"concave_frac_flow_to_perturbed_rel_perm_2_limited_{LIMIT_REL_PERM}",
...
xx = np.linspace(model._residual_saturation_w, 1 - model._residual_saturation_n, 200)
xx_hat = (xx - model._residual_saturation_w) / (
    1 - model._residual_saturation_n - model._residual_saturation_w
)
concave = (xx_hat) / (xx_hat + (1 - xx_hat) ** 3)
plt.plot(xx, concave, label="homotopy cont. init")
plt.xlabel(rf"f_w")
plt.ylabel(rf"S_w")
plt.legend()
fig.subplots_adjust(left=0.2, bottom=0.2)
plt.savefig(os.path.join(base_foldername, "homotopy_continuation.png"))


#####################
# Analyze decay rates
#####################
decays = np.linspace(0.1, 0.9, 3)  #
decays = [0.9]
homotopy_continuation_param_min = 0.1
for decay in decays:
    # Set up folder and files for logging/plots/saved time steps.
    foldername = os.path.join(
        base_foldername,
        f"homotopy_continuation_decay_{decay}",
        f"homotopy_continuation_param_min_{homotopy_continuation_param_min}",
    )
